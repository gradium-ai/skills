...
strict redirect, timeout, and download-size limits. When Tavus needs an image URL for
avatar creation, pass a short-lived asset URL that reveals no provider
credentials, and do so only in the one-time provisioning step.
...
assets go to Tavus during provisioning and synthesized audio goes to Tavus during
every session, and room media/metadata pass
