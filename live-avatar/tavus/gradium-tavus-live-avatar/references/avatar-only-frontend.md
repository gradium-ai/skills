# Avatar-first frontend

The web client exists to show and control the live conversation and to make
the pipeline legible. It is not a configuration product.

## Visible surface

Include only:

- the remote Tavus video filling the primary stage;
- remote avatar audio;
- a start/end call control;
- a microphone mute/unmute control;
- a restrained connecting, ready, listening, or speaking state;
- the live pipeline panel beside the stage, as specified in
  [live-pipeline-panel.md](live-pipeline-panel.md): five stages, one sentence
  each, one latency summary, raw events behind a collapsed Details disclosure.

Before the video arrives, the same reference portrait may fill the stage as a
placeholder. A compact error message is acceptable. Do not add a chat
transcript, character fields, voice controls, model selectors, provider
branding, or setup steps unless the user explicitly requests them. The panel
is the only diagnostic surface; do not add a second one.
...
Treat a denied or missing microphone as a degraded call, not a failed one:
keep the room connected so the avatar is still seen and heard, set the status
to say the avatar cannot hear the user, and offer a retry. Tearing the call down
on `NotAllowedError` leaves the worker greeting an empty room while the
provider bills for it.

Load `livekit-client` as a pinned 2.x UMD build (for example
`https://cdn.jsdelivr.net/npm/livekit-client@2.15.5/dist/livekit-client.umd.min.js`)
or bundle it; do not float on `@latest`.

The Tavus avatar joins as its own participant of kind `Agent` with the
attribute `lk.publish_on_behalf` set to the worker's identity, so a room
contains two agent participants. Attach video from whichever agent participant
publishes a camera track; LiveKit's `useVoiceAssistant()` React hook already
resolves this, and in plain `livekit-client` the `TrackSubscribed` handler above
is sufficient because only the avatar publishes video and audio.
